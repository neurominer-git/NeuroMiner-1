function reconX = recon_data_from_autoenc2(network, mappedX)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    for i=middle_layer + 1:no_layers
        if i ~= no_layers
            activations = [1 ./ (1 + exp(-( [network{i}.W; network{i}.bias_upW] * activations))) ones(n, 1)];
        else
            activations = [[network{i}.W; network{i}.bias_upW] * activations ones(n, 1)];
        end
    end
