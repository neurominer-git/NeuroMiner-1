function [weights, idx_feat, score] = gflipOneTime(X_train, Y_train, extra_param, cpus, labelsSet)
...
if (isfield(extra_param, 'initial_idx_feat'))
    ...
    [P_pos, P_neg, mindist_pos, mindist_neg, marginList,dists] = simbaHelper(fst_simba, X_train, X_train_square, Y_train,  idx_list, 1:m);
...
for iter = 1:max_iter,
    ...
    for ii = 1:feat_num,
        ...
        feat = perm(ii);                % Use new permutation of feature indices
        idx_list(feat)=~idx_list(feat); % flip the features
    ...
    tElapsed=toc; tTotal = tTotal + tElapsed;
    if updated==0,
        fprintf(' converged in %g iterations. ',iter);
        break;
    else
        fprintf('\nIter #%g: Score = %1.2f, # Features = %g ... done in %1.2f sec.', iter, score, sum(idx_list>0), tElapsed);
    end;
    %plot(ii,sort(weights),'b*'); drawnow;
end
fprintf('\n-------------------------------------------------------')
fprintf('\nIterations finished in %1.2f sec.',tTotal);
