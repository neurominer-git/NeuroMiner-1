...
if len(sys.argv) <= 1:
	print 'Usage: %s training_file [testing_file] [kernel_type]' % sys.argv[0]
	raise SystemExit
...
cmd = '%s -t %s -svmtrain "%s" -gnuplot "%s" "%s"' % (grid_py, kernel_type, svmtrain_exe, gnuplot_exe, scaled_file)
...
cmd = '%s -t %s -c %s -g %s "%s" "%s"' % (svmtrain_exe,kernel_type,c,g,scaled_file,model_file)
print 'Training...'
