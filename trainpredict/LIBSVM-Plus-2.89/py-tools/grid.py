...
def process_options(argv=sys.argv):
    ...
    global dataset_pathname, dataset_title, pass_through_string
    global kernel_type, svmtrain_exe, gnuplot_exe, gnuplot, out_filename, png_filename
...
class LocalWorker(Worker):
    def run_one(self,c,g):
        cmdline = '%s -t %s -c %s -g %s -v %s %s %s' % \
          (svmtrain_exe,kernel_type,c,g,fold,pass_through_string,dataset_pathname)
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x %s "cd %s; %s -t %s -c %s -g %s -v %s %s %s"' % \
          (self.host,self.cwd,
           svmtrain_exe,kernel_type,c,g,fold,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
