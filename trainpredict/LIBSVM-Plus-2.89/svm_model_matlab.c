...
// 	{
// 		
// 		rhs[out_id] = mxCreateDoubleMatrix(nBSV, 1, mxREAL);
// 		ptr = mxGetPr(rhs[out_id]);
// 		for(i = 0;i < model->lbsv; i++)
// 		{
// 			ptr[i]=model->BSV_idx[i];
// 		}
// 	}
// 	else
// 	{
...
// 	// SV_idx
// 	out_id++;
// 	rhs[out_id] = mxCreateDoubleMatrix(nSV-nBSV, 1, mxREAL);
// 	ptr = mxGetPr(rhs[out_id]);
// 	for(i = 0;i < model->l; i++)
// 	{
// 		ptr[i]=model->SV_idx[i];
// 	}
// 
// 	out_id++;
	// SVs
...
struct svm_model *matlab_matrix_to_model(const mxArray *matlab_struct, const char **msg)
{
	...
	model->param.gamma	  = (double)ptr[3];
    ...
    // w2
	model->w2 = (double*) malloc(n*sizeof(double));
	ptr = mxGetPr(rhs[id]);
	for(i=0;i<n;i++)
		model->w2[i] = ptr[i];
	id++;
...
// 	{
// 		ptr = mxGetPr(rhs[id]);
// 		model->BSV_idx = (int*) malloc((model->lbsv)*sizeof(int));
// 		for(i = 0; i < model->lbsv; i++)
// 			model->BSV_idx[i] = ptr[i];
// 	}
// 
// 	//SV_idx
// 	id++;
// 	ptr = mxGetPr(rhs[id]);
// 	model->SV_idx = (int*) malloc((model->l-model->lbsv)*sizeof(int));
// 	for(i = 0; i < model->l-model->lbsv; i++)
// 		model->SV_idx[i] = ptr[i];
