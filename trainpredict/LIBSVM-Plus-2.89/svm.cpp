...
public:
	...
	struct SolutionInfo {
		...
		double w2; // modification to get ||w||^2
	};
...
void svm_binary_svc_probability(
	const svm_problem *prob, const svm_parameter *param,
	double Cp, double Cn, double& probA, double& probB, int prob_fold)
{
	int i;
	int nr_fold = prob_fold;
	...
	else
	{
		...
		for(i=0;i<nr_class;i++)
			for(int j=i+1;j<nr_class;j++)
			{
				...
				if(param->probability)
					svm_binary_svc_probability(&sub_prob,param,weighted_C[i],weighted_C[j],probA[p],probB[p],param->probability_fold);
		...
		model->w2 = Malloc(double,nr_class*(nr_class-1)/2);
		for(i=0;i<nr_class*(nr_class-1)/2;i++)
			model->w2[i] = f[i].w2;
...
// Stratified cross validation
void svm_cross_validation_dec(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target, double *targetdec)
{
	int i;
	int *fold_start = Malloc(int,nr_fold+1);
	int l = prob->l;
	...
	for(i=0;i<nr_fold;i++)
	{
		   ...
		   (param->svm_type == C_SVC || param->svm_type == NU_SVC))
		{
			...
			{
				target[perm[j]] = svm_predict(submodel,prob->x[perm[j]]);
				svm_predict_values(submodel,prob->x[perm[j]],&targetdec[perm[j]]);
			}
		svm_destroy_model(submodel);
		free(subprob.x);
		free(subprob.y);
