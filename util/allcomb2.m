...
if iscell(varargin{1})
   ...
   for kx = 1:numel(varargin{1})
       tvarargin{kx} = varargin{1}{kx};
   end
   varargin = tvarargin;
   if exist('targn','var'), varargin{end+1} = targn; end
   q = ~cellfun('isempty',varargin) ;
end
NC = sum(q) ;
...
if any(cellfun('isempty',args)),
    ...
    A = zeros(0,NC) ;
elseif NC > 1
    if any(~cellfun(@isnumeric,args)), args = conv2cells(args); end
...
function A = conv2cells(A)

C = find(~cellfun(@iscell,A)) ;
nC = numel(C);
for i=1:nC
    if isnumeric(A{C(i)}), 
         A{C(i)} = num2cell(A{C(i)}); 
    elseif ischar(A{C(i)}),
        A{C(i)} = cellstr(A{C(i)}); 
    else
         A{C(i)} = A(C(i));
